...
public class SBoxe {
    ...
    public SBoxe(String b, int indexOfB ) {
        this.b = b;
        this.indexOfB = indexOfB;
        first = b.substring(0, 1);
        last = b.substring(5, 6);
        column = b.substring(1, 5);
    }

    public String calculate() {
        String rowByte = first + last;
        int[][] sBox = findSBox();
        int number = sBox[Integer.parseInt(rowByte, 2)][Integer.parseInt(this.column, 2)];
        ...
        return  beginningZeros+numberStr;
    }

    private int[][] findSBox(){
        switch (indexOfB){
            case 0: return S1;
            case 1: return S2;
            case 2: return S3;
